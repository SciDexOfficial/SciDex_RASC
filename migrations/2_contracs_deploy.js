...
module.exports = function(deployer, network) {
  ...
  // deployer.link(RASC_User, RASC_Store);

  deployer.deploy(RASC_Item);
  // deployer.deploy(RASC_Transaction);
  // deployer.deploy(RASC_Store, RASC_User.address);
  if (network == "local") {
    deployer.link(SafeMath, RASC_StoreTest);
    deployer.link(StringUtils, RASC_StoreTest);
    deployer.link(ArrayUtils, RASC_StoreTest);
    deployer.deploy(RASC_StoreTest);
  } else {
    deployer.link(SafeMath, RASC_Store);
    deployer.link(StringUtils, RASC_Store);
    deployer.link(ArrayUtils, RASC_Store);
